...
static bool coinFlip() {
    ...
    static std::bernoulli_distribution coin(Parameter::DEOPT_CHAOS_PROB);
    return coin(gen);
};
...
unsigned Parameter::DEOPT_CHAOS_SEED =
    getenv("PIR_DEOPT_CHAOS_SEED") ? atoi(getenv("PIR_DEOPT_CHAOS_SEED"))
                                   : std::random_device()();
double Parameter::DEOPT_CHAOS_PROB = getenv("PIR_DEOPT_CHAOS_PROB")
                                         ? atof(getenv("PIR_DEOPT_CHAOS_PROB"))
                                         : 0.05;
