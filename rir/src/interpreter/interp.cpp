...
SEXP evalRirCode(Code* c, InterpreterInstance* ctx, SEXP env,
                 const CallContext* callCtxt, Opcode* initialPC,
                 R_bcstack_t* localsBase, BindingCache* cache) {
    return evalRirCode(c, ctx, env, callCtxt, initialPC, localsBase, cache,
                       SandboxMode::None);
}

static SEXP evalRirCode(Code* c, InterpreterInstance* ctx, SEXP env,
                 const CallContext* callCtxt, Opcode* initialPC,
                 R_bcstack_t* localsBase, BindingCache* cache, SandboxMode mode) {
#define PURE_INSTRUCTION(op) INSTRUCTION(op)
#define IMPURE_INSTRUCTION(op) if (mode == SandboxMode::Sandbox) return NULL; else if (mode == SandboxMode::Record) ctx->recordUnsafe(); INSTRUCTION(op)
#define SANDBOX_NOOP_INSTRUCTION(op) if (mode == SandboxMode::Sandbox) NEXT(); INSTRUCTION(op)
#define FORCE_PROMISE(res, ctx)                                                \
    res = promiseValue(res, ctx, mode);                                        \
    if (mode == SandboxMode::Sandbox && res == NULL)                           \
        return NULL;
    ...
    BEGIN_MACHINE {
        ...
        PURE_INSTRUCTION(record_deopt_) {
            SEXP val = ostack_pop(ctx);
...
SEXP evalRirCodeExtCaller(Code* c, InterpreterInstance* ctx, SEXP env,
                          SandboxMode mode) {
    return evalRirCode(c, ctx, env, NULL, NULL, NULL, NULL, mode);
}
